...
function makeHttp(getMock: jest.Mock, postMock?: jest.Mock): HttpService {
  return { get: getMock, post: postMock ?? jest.fn() } as unknown as HttpService;
}
...
describe('DashboardController', () => {
  ...
  it('returns docs, photos, and user attributes on full success', async () => {
    ...
    const postStub = jest.fn()
      .mockReturnValueOnce(of({ data: { assets: { items: [{ id: 'ph1' }] } } })); // immich
    const ctrl = makeController(makeHttp(getStub, postStub), makeLdap());
  ...
  it('returns empty docs when Paperless fetch throws', async () => {
    ...
    const postStub = jest.fn()
      .mockReturnValueOnce(of({ data: { assets: { items: [] } } }));
    const ctrl = makeController(makeHttp(getStub, postStub), makeLdap());
    const result = await ctrl.dashboard('alice');
  ...
  it('returns empty user when LDAP getAttributes throws', async () => {
    ...
    const ctrl = makeController(makeHttp(getStub, postStub), ldap);
    const result = await ctrl.dashboard('alice');
  ...
  it('skips Immich fetch when IMMICH_API_KEY is empty', async () => {
    const getStub = jest.fn()
      .mockReturnValueOnce(of({ data: { results: [] } }));
    const postStub = jest.fn();
    const ctrl = makeController(makeHttp(getStub, postStub), makeLdap(), { IMMICH_API_KEY: '' });
    const result = await ctrl.dashboard('alice');
    expect(result.photos).toEqual([]);
    // Immich post should never be called
    expect(postStub).toHaveBeenCalledTimes(0);
  });
  ...
  it('sends x-api-key header to Immich', async () => {
    ...
    const ctrl = makeController(makeHttp(getStub, postStub), makeLdap());
    await ctrl.dashboard('alice');
    const [, , immichOpts] = postStub.mock.calls[0] as [string, unknown, { headers: Record<string, string> }];
